...
def init_estimation_params():
    """Use estimated death rate and hospitalization rate to estimate infections
    and recovered"""
...
class Dataset:
    ...
    def compartment_map(self):
        ...
        self.data['R'] = self.data['D'] / self.estimation_params['death_rate']
        self.data['I'] = self.raw_data['cases'].values - self.data['R']
        ...
        self.data['S'] -= self.data['V'] + self.data['D']
...
def latlon_to_place(api_key, lat, lon):
    ...
    with urllib.request.urlopen(uri).read() as v:
        logger.info(v)
        j = json.loads(v)
